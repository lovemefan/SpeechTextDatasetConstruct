...
from transformers.models.bert.modeling_bert import BertAttention, BertLayer, BertEncoder, BertPreTrainedModel, \
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        if config.withPhonemePitchEmbedding:
            self.bert.embeddings = BertPhonemeEmbeddings(config)
        else:
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            ...
            phoneme_pitch_ids=phoneme_pitch_ids
        )
...
class BertPhonemeEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.phoneme_pitch_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
    ...
    def forward(
        self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, phoneme_pitch_ids=None, past_key_values_length=0
    ):
        ...
        if phoneme_pitch_ids is None:
            ...
            phoneme_pitch_ids = torch.ones(input_shape, dtype=torch.long, device=self.position_ids.device)
            for i in range(input_shape[0]):
                begin = 0
                end = 0
                for j in range(input_shape[1]):
                    value = input_ids[i][j].cpu().detach().item()
                    if value in id_phoneme_pitch_map.values() and value != 1:
                        phoneme_pitch_ids[i][begin:end] *= phoneme_pitch_id_map[value]
                        begin = j
                    elif value == 1:
                        phoneme_pitch_ids[i][begin:] *= 0
                        continue
                    else:
                        end = j + 1
        ...
        phoneme_pitch_embedding = self.phoneme_pitch_embeddings(phoneme_pitch_ids)
        embeddings = inputs_embeds + token_type_embeddings + phoneme_pitch_embedding
